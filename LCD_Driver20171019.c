...
////根据自带文件修改。
////使用SPI连接芯片与LCD
////修改了原有LCD驱动程序，实现反白显示、屏幕任意行任意列显示。
...
void Show_Word(unsigned char x, unsigned char y,unsigned char ascii_word,unsigned char inverse)
{
int i=0,k=0;
	unsigned char temp;	  
	  k=(ascii_word-32)*16;	  
	  ...
	  temp=Ascii[k+i];
		if (inverse==0) WriteData(temp);  //写数据到LCD,每写完1 字节的数据后列地址自动加1
		else WriteData(~temp); 
...
void print_lcd(unsigned char line, unsigned char column, unsigned char *str,unsigned char inverse)
{
int i=column; 

  do{   
    Show_Word(line,i,*str++,inverse); 
  	i++;
  	if(i>16)
	  break;
    }
